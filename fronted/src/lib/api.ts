...
export const projectApi = {
  getProjects: (params?: {
    ...
    if (params?.teacher_id)
      query.append("teacher_id", params.teacher_id.toString());
  ...
  analyzeApplications: (params: {
    ...
    return request<ApplicationAnalysis[]>(
      `/matches/analyze?${query.toString()}`
    );
  },
};
...
export const applicationApi = {
  ...
  getMyApplications: (params?: {
    ...
    if (params?.page_size)
      query.append("page_size", params.page_size.toString());
    if (params?.fast) query.append("fast", "1");
    if (params?.scores === false) query.append("scores", "0");
